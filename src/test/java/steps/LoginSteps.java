...
public void clica_no_botão_entrar() {
    ...
    // driver = robozinho
    // find element = Procure o elemento
    // By = Por ou Pelo
    // Id = Identificador unico HTML
    // "login-button" = O nome do Id
    // "." QUANDO ACHAR ESSE ELEMENTO...
    // Click = Ação de clicar no elemento
}

@Então("deve o usuario deve ter {string}")
public void deve_o_usuario_deve_ter(String resultado) {

    if(resultado == "Login com sucesso"){ //se o valor de resultado for IGUAL  a "Login com sucesso" ENTAO execute esse comando
    ...
    }else if(resultado == "Login com falha"){ //se o valor de resultado for IGUAL  a "Login com falha" ENTAO execute esse comando
